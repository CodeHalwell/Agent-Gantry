...
export default function ProviderMatrix(){return <div className="grid">{groups.map(([name,items])=><section className="card" key={name as string}><h3>{name}</h3><ul role="list" aria-label={`${name} providers`} style={{ display: 'flex', flexWrap: 'wrap', padding: 0, margin: 0, listStyle: 'none' }}>{(items as string[]).map(i=><li key={i}><span className="pill">{i}</span></li>)}</ul></section>)}</div>}
