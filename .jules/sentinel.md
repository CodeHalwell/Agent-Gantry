...
## 2024-05-27 - [High] Regex Bypass via Newlines in Input Validation
**Vulnerability:** A regex check `r"\{\{.*\}\}"` used in `validate_description` failed to capture multi-line strings due to the default behavior of `.*` which excludes newlines. An attacker could bypass the filter by injecting newlines into their payload (e.g., `{{\nmalicious payload\n}}`).
**Learning:** `re.DOTALL` is necessary when using `.*` to validate strings that might span multiple lines, particularly for security filters meant to block specific delimiters.
**Prevention:** Always use `re.DOTALL` when implementing security filters using wildcards (`.*`) to prevent newline injection bypasses.
