...
## 2024-05-18 - Missing re.DOTALL in input validation regex
**Vulnerability:** Input validation in `validate_description` that blocked bad patterns like `{{.*}}` was vulnerable to multiline bypasses because `re.search` was used without the `re.DOTALL` flag. An attacker could inject `\n` to avoid detection.
**Learning:** Python's `.*` does not match newlines by default. In security contexts, when validating multi-line user input for malicious patterns with wildcards, not including `re.DOTALL` creates an easy bypass route.
**Prevention:** Always combine `re.IGNORECASE` and `re.DOTALL` when doing security pattern-matching using `.*` on inputs that can contain newlines.
