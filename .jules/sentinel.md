...
**Learning:** Pydantic validators run on all provided inputs, including `None` for optional fields. Security validators must be robust and type-safe. Blindly calling string operations on potentially `None` or non-string types introduces denial-of-service or application crash risks.
**Prevention:** Always ensure type safety in Pydantic validators by explicitly checking the type before applying operations (e.g., `if isinstance(v, str) and ("\n" in v or "\r" in v):`) or use `Any` as the type hint with a type check inside the validator logic.
## 2026-05-18 - [MEDIUM] Fix ToolResult status for PermissionDeniedError
**Vulnerability:** The `_check_security_policy` exception handler mapped `PermissionDeniedError` to the generic `ExecutionStatus.FAILURE` instead of using the available `ExecutionStatus.PERMISSION_DENIED` status, misclassifying authorization failures and breaking downstream security logging and logic that depends on the specific error status.
**Learning:** When creating structured responses or events (like `ToolResult`) from security checks, it is critical to use the most specific status code available (e.g., `PERMISSION_DENIED`) to ensure callers can accurately distinguish between business logic failures, timeouts, and actual security blocks.
**Prevention:** Always verify that security-related exceptions are mapped to their corresponding specific status codes or error types in the API/system response to preserve security context across system boundaries.
