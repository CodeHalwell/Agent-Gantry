...
## 2026-07-28 - [ARIA Hidden for CSS-Driven Graphics]
**Learning:** When using empty inline elements (like `<span>` or `<div>`) purely for CSS-driven decorative graphics (such as visual shapes or logos), screen readers may announce unpredictable content or pause awkwardly if they are not explicitly hidden.
**Action:** Always apply `aria-hidden="true"` to empty elements used for CSS decorative graphics to ensure assistive technologies smoothly bypass them without announcing them.
