...
## 2026-08-14 - [Decorative Elements and Screen Readers]
**Learning:** When using empty inline elements (like `<span>`) purely for CSS-driven decorative graphics (such as visual shapes or logos), screen readers can sometimes attempt to announce them in unpredictable ways or get stuck reading them as empty content.
**Action:** Always apply `aria-hidden="true"` to empty tags that exist purely for CSS decoration to ensure assistive technologies smoothly bypass them.
