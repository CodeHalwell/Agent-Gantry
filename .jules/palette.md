...
## 2026-10-26 - [Hiding Decorative CSS Graphic Spans]
**Learning:** Empty inline elements (like `<span>` or `<div>`) used purely for CSS-driven decorative graphics (such as visual shapes or logos via `class="logo"`) can confuse screen readers if not properly marked. While they are empty, they are still present in the DOM.
**Action:** Always apply `aria-hidden="true"` to empty inline elements that serve only as decorative visual elements drawn by CSS, ensuring assistive technologies smoothly bypass them without announcing unpredictable or confusing empty content.
