...
## 2026-08-13 - [Decorative Elements and Screen Readers]
**Learning:** When using empty inline elements (like `<span>` or `<div>`) purely for CSS-driven decorative graphics (such as visual shapes or logos), screen readers can sometimes announce unpredictable content or pause awkwardly on them.
**Action:** Always explicitly apply `aria-hidden="true"` to empty visual decorative elements to ensure assistive technologies smoothly bypass them.
