...
## 2026-06-25 - Avoid to_pylist() on entire PyArrow tables for dict mapping
**Learning:** When retrieving specific columns from LanceDB tables in Python (e.g., just IDs and fingerprints) to build a dictionary mapping, calling `.to_pylist()` on the entire resulting PyArrow table allocates a new dictionary object for every single row. For large datasets, this creates significant O(N) memory and time overhead.
**Action:** Use `.select(['col1', 'col2']).limit(None).to_arrow()` and directly construct the mapping by extracting and zipping the specific column lists (e.g., `dict(zip(table['col1'].to_pylist(), table['col2'].to_pylist()))`) rather than converting the entire table to a list of dicts.
