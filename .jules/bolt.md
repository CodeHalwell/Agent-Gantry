...
## 2026-08-17 - Optimize dictionary creation from PyArrow tables
**Learning:** When retrieving specific columns from LanceDB tables in Python (e.g., just IDs and fingerprints) to build a dictionary mapping, avoid calling `.to_pylist()` on the entire resulting PyArrow table, as it allocates a dictionary for every row.
**Action:** Instead, use `.select(['col1', 'col2']).limit(None).to_arrow()` and directly construct the mapping by extracting and zipping the specific column lists (e.g., `dict(zip(table['col1'].to_pylist(), table['col2'].to_pylist()))`).
