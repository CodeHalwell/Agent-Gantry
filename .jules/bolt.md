...
## 2026-07-11 - Push LanceDB operations to database engine
**Learning:** Loading entire LanceDB tables into memory via `table.to_arrow().to_pylist()` for filtering, counting, or pagination creates an O(N) memory bottleneck on large datasets.
**Action:** Use LanceDB native query builders like `search().where(condition).select([cols]).limit(limit).offset(offset).to_list()` and `count_rows(condition)` instead of pulling the data into Python lists.
