...
from agent_gantry.integrations.frameworks.autogen_live import (
    GantryWorkbench,
    gantry_workbench,
)
...
async def test_list_tools_reselects_per_turn(gantry):
    ...
    wb = gantry_workbench(gantry, limit=2)
...
async def test_call_tool_returns_tool_result_from_gantry(gantry):
    ...
    wb = gantry_workbench(gantry, limit=3)
    wb.set_query("what is the weather forecast for the city")
    await wb.list_tools()
...
async def test_lifecycle_methods_are_safe(gantry):
    wb = gantry_workbench(gantry, query="weather", limit=1)
    await wb.start()
    state = await wb.save_state()
    assert state["query"] == "weather"

    wb2 = gantry_workbench(gantry, limit=1)
