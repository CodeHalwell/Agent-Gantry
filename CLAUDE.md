...
Subcommands: `list`, `search`, `lint`, `sim` (pairwise similarity), `sync`, `serve-mcp` (expose the registry as an MCP server over stdio, Streamable HTTP or SSE), and `install-skill` (installs the bundled Claude Skill, e.g. `agent-gantry install-skill --claude`).
...
- Version of record is `pyproject.toml` / `agent_gantry/__init__.py` `__version__` (currently `0.18.0`). The docs `package.json` and `README.md` may lag — `tests/test_version_consistency.py` guards consistency where it matters.
